"""HVAC Cost Resolver -- deterministic price corridor lookup.

Matches a quotation line item description against the HVAC cost catalog
(GCC market rates, AED, 2024-2025).

Matching strategy:
  1. Exact category_code match
  2. Keyword fuzzy match against catalog descriptions
    3. Fallback to AI estimation when no match found
"""
...
class HVACCostResolver:
    """Resolve market cost corridor for an HVAC line item.

    Usage::
        result = HVACCostResolver.resolve(description, category_code, unit)
    ...
    def resolve(
        ...
        """Return cost corridor for the given line item.
        ...
        if category_code and category_code.upper() in HVAC_BENCHMARK_CATALOG:
            ...
            return HVACCostResolver._format_result(entry, category_code.upper(), 100)
        ...
        if best_key and best_score >= 2:
            ...
            logger.debug(
                "HVAC cost catalog matched '%s' -> %s (score=%d)", description[:60], best_key, best_score
            )
            return HVACCostResolver._format_result(entry, best_key, best_score)

        # 3. No match
        logger.info("HVAC cost catalog: no match for description '%s'", description[:80])
        return {
            "min": None,
            "avg": None,
            "max": None,
            ...
            "catalog_key": None,
            "match_score": 0,
            "reasoning": "No matching HVAC cost catalog entry found for this line item. Will use AI estimation.",
        }
    ...
    def _format_result(entry: Dict[str, Any], key: str, score: int) -> Dict[str, Any]:
        return {
            ...
            "reasoning": (
                f"Matched HVAC cost catalog entry '{key}' "
...
# Backward-compat alias for any remaining external imports.
HVACBenchmarkResolver = HVACCostResolver
