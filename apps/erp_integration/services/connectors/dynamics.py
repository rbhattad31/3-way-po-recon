"""Microsoft Dynamics ERP Connector — stub for future implementation."""
...
_UNSUPPORTED_LOOKUP = ERPResolutionResult(
    resolved=False,
    source_type=ERPSourceType.NONE,
    reason="Dynamics connector not yet implemented",
)

_UNSUPPORTED_SUBMIT = ERPSubmissionResult(
    success=False,
    status="UNSUPPORTED",
    error_message="Dynamics connector not yet implemented",
)


class DynamicsConnector(BaseERPConnector):
    """Stub connector for Microsoft Dynamics 365."""
    ...
    def lookup_vendor(self, vendor_code: str = "", vendor_name: str = "", **kwargs) -> ERPResolutionResult:
        return ERPResolutionResult(resolved=False, source_type=ERPSourceType.NONE,
                                   reason="Dynamics connector not yet implemented", connector_name=self.connector_name)
    ...
    def create_invoice(self, payload: Dict[str, Any]) -> ERPSubmissionResult:
        return ERPSubmissionResult(success=False, status="UNSUPPORTED",
                                   error_message="Dynamics connector not yet implemented", connector_name=self.connector_name)
